...
int main(int argc, char *argv[])
{
  ...
  if (taskid == MASTER)
  {
    ...
    for (dest = 1; dest < numtasks; dest++)
    {
      ...
      MPI_Send(&data[offset], chunksize, MPI_LONG, dest, tag2, MPI_COMM_WORLD);
    ...
    MPI_Reduce(&mysum, &sum, 1, MPI_LONG, MPI_SUM, MASTER, MPI_COMM_WORLD); // every processor needs to reduce in the order to get final sum
  ...
  if (taskid > MASTER)
  {
    ...
    dest = MASTER;
    MPI_Send(&offset, 1, MPI_INT, dest, tag1, MPI_COMM_WORLD);
    MPI_Send(&data[offset], chunksize, MPI_LONG, MASTER, tag2, MPI_COMM_WORLD);
