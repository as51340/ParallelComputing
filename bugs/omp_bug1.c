/******************************************************************************
* FILE: omp_bug1.c
* DESCRIPTION:
*   This example attempts to show use of the parallel for construct.  However
*   it will generate errors at compile time.  Try to determine what is causing
*   the error.  See omp_bug1fix.c for a corrected version.
* AUTHOR: Blaise Barney  5/99
* LAST REVISED: 04/06/05
* Modified by Emil Loevbak
******************************************************************************/
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
#define N 50
#define CHUNKSIZE 5

int main(int argc, char *argv[])
{
  int i, chunk, tid;
  float a[N], b[N], c[N];

  /* Some initializations */
  for (i = 0; i < N; i++)
    a[i] = b[i] = i * 1.0;
  chunk = CHUNKSIZE;
  char first_time = 'y';

// or it also maybe better to set int i in the loop
#pragma omp parallel for shared(a, b, c, chunk) private(i, tid) firstprivate(first_time) schedule(static, chunk)
  {
    for (i = 0; i < N; i++)
    {
      if(first_time == 'y') {
        tid = omp_get_thread_num();
        first_time = 'n';
      }
      c[i] = a[i] + b[i];
      printf("Thread id= %d i= %d c[i]= %f\n", tid, i, c[i]);
    }
  } /* end of parallel for construct */
}
